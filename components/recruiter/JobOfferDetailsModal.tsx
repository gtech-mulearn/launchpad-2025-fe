...
export const JobOfferDetailsModal: React.FC<JobOfferDetailsModalProps> = ({
  ...
  useEffect(() => {
    if (eligibleCandidatesData && selectedJobOffer) {
      eligibleCandidatesData.response.data.forEach((candidate: Candidate) => {
        ...
        if (
          (candidate.application_status === "invited" || candidate.application_status === "interview_scheduled") &&
          !existingRequest
        ) {
          const newInvite: JobInvite = {
            ...
            application_id: `${candidate.id}-${selectedJobOffer.id}-${Date.now()}`, // Placeholder; ideally from API
  ...
  const getCandidateStatus = (candidate: Candidate, hireRequest?: JobInvite) => {
    ...
    if (candidate.application_status === "invited" && candidate.application_timeline?.applied_at) return "accepted";
                                ...
                                {status === "interview" && hireRequest && (
                                  <>
                                    ...
                                    <Button
                                      ...
                                      onClick={() => onHireCandidate(hireRequest.id)}
                                    >
